...
// import { useNavigate, useLocation } from "react-router-dom"; 
// import axios from "axios";
// import { FiUser, FiMail, FiLock, FiFacebook, FiGithub, FiKey } from "react-icons/fi";
// import { FcGoogle } from "react-icons/fc";
// import { toast } from "react-toastify";
// import Navbar from "../components/Navbar";
// import Footer from "../components/Footer";
...
//         .link-text {
//           color: #333; fontSize: 14px; text-decoration: none; margin: 15px 0; font-weight: 500; cursor: pointer; transition: 0.2s;
//         }
//         .link-text:hover { color: ${colors.primary}; }
//       `}</style>
...
//                     <div style={{ width: "100%", position: "relative" }}>
//                       <FiMail style={{ position: "absolute", top: "18px", left: "15px", color: "#bbb" }} />
//                       <input type="email" placeholder="Enter your email" className="modern-input" 
...
//                     <div style={{ width: "100%", position: "relative" }}>
//                       <FiLock style={{ position: "absolute", top: "18px", left: "15px", color: "#bbb" }} />
//                       <input type="password" placeholder="New Password" className="modern-input" 
...
function AuthPage() {
  ...
  const colors = { primary: "#E76F51", secondary: "#264653", bg: "#FDFCF8", white: "#FFFFFF" };
  ...
  const handleGoogleLogin = () => window.open(`${BASE_URL}/google`, "_self");
  ...
  const handleSendOTP = async (e) => {
    ...
    setLoading(true);
    try {
      // FIX: Ensure no space at the end of the URL string!
      const res = await axios.post(`${BASE_URL}/forgot-password`, { email: forgotEmail });
      ...
      console.error("OTP Error:", err);
      // Backend error message-ai toast-il kaattum
  ...
  return (
    <div style={{ backgroundColor: colors.bg, minHeight: "100vh", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;700&family=Playfair+Display:wght@600;800&display=swap');
        .container { position: relative; overflow: hidden; width: 1000px; max-width: 100%; min-height: 650px; background-color: #fff; border-radius: 30px; box-shadow: 0 25px 50px rgba(0,0,0,0.1); }
        ...
        .overlay-container { position: absolute; top: 0; left: 50%; width: 50%; height: 100%; overflow: hidden; transition: transform 0.6s ease-in-out; z-index: 100; border-top-right-radius: 30px; border-bottom-right-radius: 30px; }
        .container.right-panel-active .overlay-container { transform: translateX(-100%); border-radius: 30px 0 0 30px; }
        .overlay { background: #E76F51; background: linear-gradient(to right, #E76F51, #264653); background-repeat: no-repeat; background-size: cover; background-position: 0 0; color: #ffffff; position: relative; left: -100%; height: 100%; width: 200%; transform: translateX(0); transition: transform 0.6s ease-in-out; }
        .container.right-panel-active .overlay { transform: translateX(50%); }
        ...
        .overlay-right { right: 0; transform: translateX(0); }
        .container.right-panel-active .overlay-right { transform: translateX(20%); }
        .modern-input { background-color: #F5F5F5; border: 2px solid transparent; padding: 15px 15px 15px 45px; width: 100%; border-radius: 12px; margin-bottom: 15px; font-size: 0.95rem; outline: none; transition: 0.3s; }
        .modern-input:focus { background: white; border-color: ${colors.primary}; box-shadow: 0 5px 15px rgba(231,111,81,0.1); }
        .btn-primary { border-radius: 25px; border: none; background-color: ${colors.primary}; color: #ffffff; font-size: 14px; font-weight: bold; padding: 15px 45px; letter-spacing: 1px; text-transform: uppercase; transition: transform 80ms ease-in; cursor: pointer; margin-top: 10px; box-shadow: 0 10px 20px rgba(231,111,81,0.3); }
        .btn-primary:active { transform: scale(0.95); }
        .btn-primary:disabled { background-color: #ccc; cursor: not-allowed; box-shadow: none; }
        .btn-ghost { background-color: transparent; border-color: #ffffff; border: 2px solid white; border-radius: 25px; color: white; padding: 12px 40px; font-weight: bold; cursor: pointer; transition: 0.3s; }
        .btn-ghost:hover { background: white; color: ${colors.primary}; }
        .social-btn { border: 1px solid #DDDDDD; border-radius: 50%; display: inline-flex; justify-content: center; align-items: center; margin: 0 5px; height: 45px; width: 45px; cursor: pointer; transition: 0.3s; }
        .social-btn:hover { border-color: ${colors.primary}; transform: translateY(-3px); }
        .bg-image { position: absolute; inset: 0; z-index: -1; background-image: url(${cakeBg}); background-size: cover; background-position: center; opacity: 0.4; mix-blend-mode: overlay; }
        .link-text { color: #333; fontSize: 14px; text-decoration: none; margin: 15px 0; font-weight: 500; cursor: pointer; transition: 0.2s; }
        .link-text:hover { color: ${colors.primary}; }
      `}</style>
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "100px 20px" }}>
        <div className={`container ${isSignUpMode ? "right-panel-active" : ""}`}>
          <div className="form-container sign-up-container">
            <form onSubmit={handleRegister} style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 50px", backgroundColor: "white" }}>
              ...
              <span style={{ fontSize: "12px", marginBottom: "15px", color: "#888" }}>or use your email for registration</span>
              <div style={{ width: "100%", position: "relative" }}><FiUser style={{ position: "absolute", top: "18px", left: "15px", color: "#bbb" }} /><input type="text" name="name" placeholder="Name" className="modern-input" value={regData.name} onChange={handleRegChange} /></div>
              <div style={{ width: "100%", position: "relative" }}><FiMail style={{ position: "absolute", top: "18px", left: "15px", color: "#bbb" }} /><input type="email" name="email" placeholder="Email" className="modern-input" value={regData.email} onChange={handleRegChange} /></div>
              <div style={{ width: "100%", position: "relative" }}><FiLock style={{ position: "absolute", top: "18px", left: "15px", color: "#bbb" }} /><input type="password" name="password" placeholder="Password" className="modern-input" value={regData.password} onChange={handleRegChange} /></div>
              <button className="btn-primary" disabled={loading}>{loading ? "Creating..." : "Sign Up"}</button>
            </form>
          </div>
          <div className="form-container sign-in-container">
            {!forgotPasswordMode ? (
              <form onSubmit={handleLogin} style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 50px", backgroundColor: "white" }}>
                ...
                <span style={{ fontSize: "12px", marginBottom: "15px", color: "#888" }}>or use your account</span>
                <div style={{ width: "100%", position: "relative" }}><FiMail style={{ position: "absolute", top: "18px", left: "15px", color: "#bbb" }} /><input type="email" name="email" placeholder="Email" className="modern-input" value={loginData.email} onChange={handleLoginChange} /></div>
                <div style={{ width: "100%", position: "relative" }}><FiLock style={{ position: "absolute", top: "18px", left: "15px", color: "#bbb" }} /><input type="password" name="password" placeholder="Password" className="modern-input" value={loginData.password} onChange={handleLoginChange} /></div>
                <span className="link-text" onClick={() => setForgotPasswordMode(true)}>Forgot your password?</span>
                <button className="btn-primary" disabled={loading}>{loading ? "Signing In..." : "Sign In"}</button>
              </form>
            ) : (
              <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 50px", backgroundColor: "white" }}>
                <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: "2.2rem", margin: "0 0 20px 0", color: colors.secondary }}>{otpSent ? "Reset Password" : "Forgot Password"}</h1>
                {!otpSent ? (
                  <form onSubmit={handleSendOTP} style={{ width: "100%", display: "flex", flexDirection: "column", alignItems: "center" }}>
                    <p style={{ fontSize: "14px", color: "#666", marginBottom: "20px", textAlign: "center" }}>Enter your email address to receive an OTP.</p>
                    <div style={{ width: "100%", position: "relative" }}><FiMail style={{ position: "absolute", top: "18px", left: "15px", color: "#bbb" }} /><input type="email" placeholder="Enter your email" className="modern-input" value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)} required /></div>
                    <button className="btn-primary" disabled={loading}>{loading ? "Sending..." : "Send OTP"}</button>
                  </form>
                ) : (
                  <form onSubmit={handleResetPassword} style={{ width: "100%", display: "flex", flexDirection: "column", alignItems: "center" }}>
                    <div style={{ width: "100%", position: "relative" }}><FiKey style={{ position: "absolute", top: "18px", left: "15px", color: "#bbb" }} /><input type="text" placeholder="Enter OTP" className="modern-input" value={otp} onChange={(e) => setOtp(e.target.value)} required /></div>
                    <div style={{ width: "100%", position: "relative" }}><FiLock style={{ position: "absolute", top: "18px", left: "15px", color: "#bbb" }} /><input type="password" placeholder="New Password" className="modern-input" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} required /></div>
                    <button className="btn-primary" disabled={loading}>{loading ? "Resetting..." : "Reset Password"}</button>
                  </form>
                )}
                <span className="link-text" style={{ marginTop: "20px" }} onClick={() => { setForgotPasswordMode(false); setOtpSent(false); }}>Back to Login</span>
              </div>
            )}
          </div>
          <div className="overlay-container"><div className="overlay"><div className="bg-image"></div><div style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg, rgba(231, 111, 81, 0.9), rgba(38, 70, 83, 0.8))" }}></div><div className="overlay-panel overlay-left"><h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: "2.5rem", margin: 0, fontWeight: "bold" }}>Welcome Back!</h1><p style={{ fontSize: "14px", fontWeight: 300, lineHeight: "20px", letterSpacing: "0.5px", margin: "20px 0 30px" }}>To keep connected with us please login with your personal info</p><button className="btn-ghost" onClick={() => setIsSignUpMode(false)}>Sign In</button></div><div className="overlay-panel overlay-right"><h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: "2.5rem", margin: 0, fontWeight: "bold" }}>Hello, Friend!</h1><p style={{ fontSize: "14px", fontWeight: 300, lineHeight: "20px", letterSpacing: "0.5px", margin: "20px 0 30px" }}>Enter your personal details and start your delicious journey with us</p><button className="btn-ghost" onClick={() => setIsSignUpMode(true)}>Sign Up</button></div></div></div>
        </div>
      </div>
      <Footer />
    </div>
